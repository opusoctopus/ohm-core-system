...
Public Class FormCheckRight
    ...
    Sub LoadData()
        Try
            str = "SELECT txn_id, maker, n_type, topbott, start_time, run_time, end_time, status FROM prodsys2_solder_paste_mgt_coolingcase_tbl WHERE txn_id = '" & TextBox1.Text & "' AND status = 'AVAILABLE'"
    ...
    Sub CheckFIFO()
        If ComboBox1.Text = "TOP" Then
            Call connection()
            cmd = New MySql.Data.MySqlClient.MySqlCommand("SELECT txn_id FROM prodsys2_solder_paste_mgt_checkright_tbl WHERE txn_id IN (SELECT MAX(txn_id) FROM prodsys2_solder_paste_mgt_checkright_tbl WHERE topbott='TOP')", conn)
            Dim UrutanKode As String
            Dim Hitung As Long
            rd = cmd.ExecuteReader
            rd.Read()
            If rd.HasRows Then
                Label4.Text = rd("txn_id")
                If Len(Label4.Text) > 10 Then
                    Label4.Text = Strings.Left(Label4.Text, 13)
                End If
                Hitung = Microsoft.VisualBasic.Right(rd.GetString(0), 3) + 1
                UrutanKode = Label4.Text + Microsoft.VisualBasic.Right("000" & Hitung, 3)
                Label5.Text = UrutanKode
                rd.Close()
                conn.Close()
            End If
        ElseIf ComboBox1.Text = "BOTTOM" Then
            Call connection()
            cmd = New MySql.Data.MySqlClient.MySqlCommand("SELECT txn_id FROM prodsys2_solder_paste_mgt_checkright_tbl WHERE txn_id IN (SELECT MAX(txn_id) FROM prodsys2_solder_paste_mgt_checkright_tbl WHERE topbott='BOTTOM')", conn)
            Dim UrutanKode As String
            Dim Hitung As Long
            rd = cmd.ExecuteReader
            rd.Read()
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        For Baris As Integer = 0 To DataGridView1.Rows.Count - 1
            rd.Close()
            str = "SELECT * FROM prodsys2_solder_paste_mgt_coolingcase_tbl WHERE txn_id='" & TextBox1.Text & "'"
        ...
        Try
            If rd.HasRows Then
                ...
                If ComboBox2.Text = "" Then
                    MsgBox("PLEASE SELECT HOLE!", vbInformation)
                Else
                    conn.Open()
                    str = "SELECT hole FROM prodsys2_solder_paste_mgt_checkright_tbl WHERE hole='" & ComboBox2.Text & "'"
                    cmd = New MySql.Data.MySqlClient.MySqlCommand(str, conn)
                    rd = cmd.ExecuteReader
                    rd.Read()
                    Try
                        If rd.HasRows Then
                            MsgBox("HOLE [" & ComboBox2.Text & "] ALREADY USED!", vbExclamation)
                        Else
                            rd.Close()
                ...
                conn.Dispose()
            Else
                MsgBox("SOLDER PASTE NOT AVAILABLE IN COOLING CASE!", vbInformation)
                Call bersih()
                rd.Close()
                conn.Close()
                conn.Dispose()
