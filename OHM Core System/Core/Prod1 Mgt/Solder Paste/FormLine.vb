...
Public Class FormLine
    ...
    Sub LoadData()
        Try
            str = "SELECT txn_id, maker, n_type, topbott, start_time, run_time, end_time, status FROM prodsys2_solder_paste_mgt_mixer_tbl WHERE txn_id = '" & TextBox1.Text & "' AND status = 'ALREADY MIXER'"
    ...
    Sub CheckFIFO()
        If ComboBox1.Text = "TOP" Then
            Call connection()
            cmd = New MySql.Data.MySqlClient.MySqlCommand("SELECT txn_id FROM prodsys2_solder_paste_mgt_line_tbl WHERE txn_id IN (SELECT MAX(txn_id) FROM prodsys2_solder_paste_mgt_line_tbl WHERE topbott='TOP')", conn)
        ...
        ElseIf ComboBox1.Text = "BOTTOM" Then
            Call connection()
            cmd = New MySql.Data.MySqlClient.MySqlCommand("SELECT txn_id FROM prodsys2_solder_paste_mgt_cline_tbl WHERE txn_id IN (SELECT MAX(txn_id) FROM prodsys2_solder_paste_mgt_line_tbl WHERE topbott='BOTTOM')", conn)
            Dim UrutanKode As String
            Dim Hitung As Long
            rd = cmd.ExecuteReader
            rd.Read()
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        Call connection()
        str = "SELECT * FROM prodsys2_solder_paste_mgt_mixer_tbl WHERE txn_id='" & TextBox1.Text & "' AND status = 'AVAILABLE'"
        cmd = New MySql.Data.MySqlClient.MySqlCommand(str, conn)
        rd = cmd.ExecuteReader
        rd.Read()
        Try
            If rd.HasRows Then
                ...
                If ComboBox2.Text = "" Then
                    MsgBox("PLEASE SELECT LINE!", vbInformation)
                Else
                    conn.Open()
                    str = "SELECT line FROM prodsys2_solder_paste_mgt_line_tbl WHERE line='" & ComboBox2.Text & "' AND status ='AVAILABLE'"
                    cmd = New MySql.Data.MySqlClient.MySqlCommand(str, conn)
                    rd = cmd.ExecuteReader
                    rd.Read()
                    Try
                        If rd.HasRows Then
                            MsgBox("SOLDER PASTE IS ALREADY IN [" & ComboBox2.Text & "]", vbExclamation)
                        Else
                            rd.Close()
                            cmd = New MySql.Data.MySqlClient.MySqlCommand
                            cmd.Connection = conn
                            str = "INSERT INTO `prodsys2_solder_paste_mgt_line_tbl`(`txn_id`, `maker`, `n_type`, `topbott` , `line`, `start_time`, `status`, `pic`) VALUES (@txn_id,@maker,@n_type,@topbott,@line,@start_time,@status,@pic)"
                ...
                conn.Dispose()
            Else
                MsgBox("SOLDER PASTE STILL MIXING PROCESS!", vbInformation)
                Call bersih()
                rd.Close()
                conn.Close()
                conn.Dispose()
